...
### Source
Hardcoded values in the MODEL layer.  
Derived from Medicaid business rules and HCPCS patterns.
...
| Column             | Type         | Description                                      |
|--------------------|--------------|--------------------------------------------------|
| `CATEGORY_KEY`     | INTEGER      | Surrogate key for category                       |
| `SERVICE_CATEGORY` | VARCHAR(10)  | Category label (`RX`, `ED`, `IP`, `OP`, `OTHER`) |
| `DESCRIPTION`      | VARCHAR(50)  | Human‑readable description                        |

---

## Values Loaded Into the Dimension

| CATEGORY_KEY | SERVICE_CATEGORY | DESCRIPTION            |
|--------------|------------------|------------------------|
...
## Transformation Logic (Model Layer)

> **Note:**  
> The FACT table currently assigns only **RX**, **OP**, and **OTHER**.  
> `ED` and `IP` categories exist in the dimension for **future expansion** (e.g., POS‑based classification).  
> This is intentional and documented for future enhancement.

### Current FACT Derivation Logic

```sql
CASE 
    WHEN HCPCS_CODE LIKE 'J%'  THEN 'RX'      -- Injectable drugs
    WHEN HCPCS_CODE LIKE 'A0%' THEN 'OP'      -- Outpatient
